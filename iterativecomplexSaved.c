                         ...
                         suncomplextype* new_vk_norm)
{
  ...
  for (i = i0; i < k; i++)
  {
    ...
    h[i][k_minus_1] = N_VDotProd_SComplex(v[i], v[k]); //Amihere
    // SUNCheckLastErr();
  ...
  *new_vk_norm = SUNRsqrt((sunrealtype)*new_vk_norm);
  ...
  for (i = i0; i < k; i++)
  {
    ...
    new_product = N_VDotProd_SComplex(v[i], v[k]); //Amihere
    // SUNCheckLastErr();
...
int SUNQRfactComplex(int n, suncomplextype** h, suncomplextype* q, int job)
{
  ...
  default:
    ...
    for (k = 0; k < n_minus_1; k++)
    {
      ...
      h[k + 1][n_minus_1] = conj(s) * temp1 + c * temp2;
    }
